...
\end{align}
For electrons, I have multiplied the particle current in Eq.
(@eq:cps:electron-current) by the electron's charge $e$ to have an electric
current.
...
### Large bias limit and stationary current in the absence of resonators 
\label{sec:cps:absence-resonators}
...
contribution to the current (exemplified by the results of Sec.
\ref{sec:cps:absence-resonators}) where electrons flow into the leads without
